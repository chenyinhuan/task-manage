			...
			<el-table-column :prop="item.prop" :label="item.label" :width="item.width"
				...
				<template slot-scope="scope">
					...
					<div v-if="item.slot && item.prop=='opt'">
						...
						<el-button type="text" v-if="scope.row.taskState == 3 || scope.row.taskState == 4"
							@click="addTask(scope.row)">编辑</el-button>
						<el-button type="text" v-if="scope.row.taskState == 1 || scope.row.taskState == 2"
							@click="cancelCurTask(scope.row)">取消</el-button>
						<el-button type="text" @click="viewDes(scope.row)">查看说明</el-button>
						<el-button type="text"
							v-if="(scope.row.createUserId!=userInfo.userId && scope.row.taskState == 1) || scope.row.createUserId!=userInfo.userId && scope.row.taskState == 2"
