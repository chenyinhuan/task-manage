<template>
  <div id="addTask">
    ...
    <section>
      <p>任务说明</p>
      <el-input :disabled="isDisabled" v-model="form.description" placeholder="请输入任务说明" maxlength="200" show-word-limit>
      </el-input>
    </section>
    <section>
      <p>任务模版</p>
    ...
    <section>
      ...
      <div>
        <el-date-picker :disabled="isDisabled" v-model="form.startTime" type="datetime"
          value-format="yyyy-MM-dd HH:mm:ss" :picker-options="startTimeRule" :clearable="false" placeholder="选择日期">
        </el-date-picker>
        <span style="margin: 0px 3px;">-</span>
        <el-date-picker :disabled="isDisabled" v-model="form.endTime" type="datetime" :clearable="false"
          :picker-options="endTimeRule" value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择日期">
        </el-date-picker>
      </div>
    ...
    <section>
      <p>派发名单</p>
      <div class="assigment">
        <span v-for="(item,index) in form.users" :key="index" style="margin-right: 10px"
          v-if="!isDisabled">{{item.username}}</span>
        <span v-for="(item,index) in selectedData" :key="index" v-if="isDisabled" style="margin-right: 10px">{{item.username}}</span>
    ...
    <div class="foot" v-if="!isDisabled">
