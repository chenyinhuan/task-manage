    ...
    <section v-for="(item, index) in taskTplTargetVOs" :key="index" v-if="curIndex == index">
        ...
        <div class="content" v-if="item.taskTplTargetEntity.targetResultShowType[0] == 1">
          ...
          <div class="container">
                  ...
                  <div>
                    <div v-for="(sitem,sindex) in citem.taskTplTargeifExtEntityList" :key="sindex">
                      ...
                      <el-select v-model="sitem.targetStartId" placeholder="选择指标">
                        <el-option v-for="(titem, tindex) in targetList" :value="titem.targetName" :label="titem.targetName"
                      ...
                      <el-input-number controls-position="right" style="width: 140px;margin-right: 10px;" v-model="sitem.targeDefineValue" v-if="sitem.targetEndId != '' && sitem.targetEndId == 0"></el-input-number>
            ...
            <div class="else" v-if="elseStatus">
              ...
              <div style="position: relative;">
                <el-select v-model="item.taskTplTargeelseEntity.ifResult" placeholder="选择输出类型">
                  <el-option v-for="(eitem, eindex) in complateList" :label="eitem.label" :value="eitem.value" :key="eindex"></el-option>
                </el-select>
                <el-input v-model="item.taskTplTargeelseEntity.defineValue" v-if="item.taskTplTargeelseEntity.ifResult == 3"></el-input>
                <div style="border: 1px solid rgb(217, 217, 217);border-radius: 4px;z-index: 2;width: 60px;height: 32px;background: #ffffff;padding: 0px 6px;"  v-if="item.taskTplTargeelseEntity.ifResult == 3"><colorPicker v-model="item.taskTplTargeelseEntity.color"/></div>
                <el-checkbox style="margin: 0px 30px" :label="0" v-model="item.taskTplTargeelseEntity.elseResultShow">不显示数值</el-checkbox>
                <el-checkbox-group :style="{'position': item.taskTplTargeelseEntity.ifResult == 3?'absolute':'',
                'margin-top': item.taskTplTargeelseEntity.ifResult == 3?'70px':''}" v-model="item.taskTplTargeelseEntity.elseResultShowType" @change="bindCheckBox(item.taskTplTargeelseEntity.elseResultShowType)">
                  <el-checkbox :label="1">显示百分数（两位小数)</el-checkbox>
                  <el-checkbox :label="2">显示数值（两位小数）</el-checkbox>
                </el-checkbox-group>
              </div>
      ...
      <section class="end-time">
        ...
        <div class="date">
          ...
          <el-date-picker type="datetime" v-if="item.taskTplTargetEntity.targetTestTimeType == 2" v-model="item.taskTplTargetEntity.targetTestDate"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      save() {
        let flag = true;
        this.taskTplTargetVOs.forEach(item => {
          if(!item.taskTplTargeelseEntity.ifResult) {
            flag = false;
            return this.$message.warning('请选择否则的输出类型')
          }
        })
        if(flag == false) return;
        let params = {
          taskTplTargetVOs: this.taskTplTargetVOs,
