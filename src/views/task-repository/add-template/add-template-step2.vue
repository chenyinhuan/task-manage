<template>
  <div id="step2">
    ...
    <section v-for="(item, index) in taskTplTargetVOs" :key="index" v-if="curIndex == index">
      <section>
        <p>任务指标名称</p>
        <el-input class="input" v-model="item.taskTplTargetEntity.taskName" placeholder="请输入任务指标名称" maxlength="20" show-word-limit></el-input>
      </section>
      <section>
        <p>任务指标计算</p>
        ...
        <div class="content" v-if="item.taskTplTargetEntity.targetResultShowType == 1">
          <h3>逻辑判断</h3>
          <div class="container">
  ...
  export default {
    ...
    data() {
      return {
        ...
        taskTplTargetVOs: [{
          ...
          "taskTplTargetEntity": {
            "targetName": "", //任务名称
            "targetResultShowType": '', //状态  1：判断后输出 2：直接输出
    ...
    methods: {
      ...
      changeResShowType(value) {
        console.log(value)
        if (this.taskTplTargetVOs[curIndex].taskTplTargetEntity.targetResultShowType.length > 1) {
          this.taskTplTargetVOs[curIndex].taskTplTargetEntity.targetResultShowType.splice(0, 1)
        }
      },
      bindCheckBox(value) {
