		...
		<section class="container">
			<div class="content">
				<div class="content-item" v-for="(item, index) in taskRecordDetailBasicVOs" :key="index"
					...
					<el-date-picker v-if="item.formType == 1 && item.dataType == 4" value-format="yyyy-MM-dd"
					...
					<el-select placeholder="请输入" v-model="item.fieldValue"
						...
						<el-option v-for="citem in item.enums" :key="citem.id" :label="citem.enumValue"
							:value="citem.id">
						</el-option>
					</el-select>
					<div v-if=" item.formType == 4" style="display: flex;align-items: center;">
						...
						<el-upload accept=".jpg,.png,.jpeg" :style="{'margin-left':item.fieldValue?'24px':''}"
							class="upload-demo" drag :action="uploadUrl" :show-file-list="false"
							:with-credentials="true" :on-success="successUpload" :before-upload="beforeUpload">
		...
		created() {
			...
			if (this.isEdit == 1) { // 编辑
				...
				getTaskRecord(params).then(res => {
					...
					let tempJson = JSON.parse(JSON.stringify(res.taskRecordVO.taskRecordDetailBasicVOs))
					let params1 = {
						taskId: this.taskId
					}
					
					getRecordListInputs(params1).then(res1 => {
						if (res1.code == 0) {
							res1.taskRecords.taskRecordDetailBasicVOs.forEach((item, index) => {
								if (item.formType == 2 || item.formType == 3) {
									tempJson[index].enums = item
										.enums;
									if (item.formType == 2) tempJson[index].fieldValue = Number(tempJson[index].fieldValue);
									if (item.formType == 3 && tempJson[index].fieldValue) {
										let value = res.taskRecordVO.taskRecordDetailBasicVOs[index].fieldValue.split('@#');
										if(value.length) {
											tempJson[index].fieldValue = value.map(Number);
										}else tempJson[index].fieldValue = [];
					...
					console.log(res.taskRecordVO.taskRecordDetailBasicVOs)
					this.taskRecordDetailBasicVOs = [...[], ...tempJson]
				})
					...
					updateTaskRecord(params).then(res => {
						if (res.code == 0) {
							...
							if (this.$route.path.indexOf('/my-assignment/') != -1) {
								...
								`/task-center/task-dtl-list?id=${this.taskId}&date=${this.date}`)
